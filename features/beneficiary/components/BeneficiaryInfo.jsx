...
const BeneficiairyInfo = (props) => {
	...
	const onDownload = async () => {
		...
		if (typeof link.download === 'string') {
			link.href = data;
			const now = new Date();
			link.download = 'CARD_' + props.family_name + '_ON_' + now.toLocaleDateString() + '_AT_' + now.getHours() + ':' + now.getMinutes() + ':' + now.getSeconds() + '.jpg';
	...
	return (
		<>
			...
			<div class='grid grid-cols-1 md:grid-cols-1 gap-x-4 gap-y-1 my-7'>
				<div
					class='col-start-1 mx-auto'
					ref={cardPdf}
				>
					<div class='h-[273px] w-[432px] shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px]'>
								...
								<div class='grid grid-cols-4 gap-x-4 gap-y-1 '>
									<div class='my-auto'>
										<img
											...
											src={props.picture}
											alt='Preview'
											style={{ 'max-width': '90px', 'max-height': '90px', 'object-fit': 'fill', overflow: 'hidden' }}
										/>
									</div>
									...
									<div class='col-span-2 text-[10px] font-semibold'>
										<div class='grid grid-cols-5 gap-x-4 gap-y-1 p-3 justify-self-center'>
											...
											<div class='col-span-2 font-semibold'>{props.doe || 'N.A'}</div>
										</div>
									</div>
									<div class='grid grid-col-3 gap-1 px-3 py-1'>
										...
										<div class='col-span-2 mx-auto'>
											<div class='text-[7px] w-full'>Signature et cachet</div>
										</div>
										<div class='col-span-2 mx-auto'>
											<img
												class='w-full h-full object-cover'
												src={stamp}
												alt='Preview'
												style={{ 'max-width': '50px', 'max-height': '50px', 'object-fit': 'fill', overflow: 'hidden' }}
											/>
										</div>
									</div>
									...
									{/* <div class='my-auto justify-self-center'>
										<canvas
											...
											ref={qrCodeCanvasRef}
										/>
										<div class='text-[7px] text-center w-full'>Signature et cachet</div>

										<img
											class='w-full h-full object-cover items-center self-center'
											src={stamp}
											alt='Preview'
